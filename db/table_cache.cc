...
TableCache::TableCache(const std::string &dbname, const Options *options,
  ...
  if (options->opEp_.useLRUCache) {
    ...
    cache_ = NewMultiQueue(
        dbname, entries, options->opEp_.lrus_num_, options->opEp_.life_time,
        options_->opEp_.change_ratio, options_->opEp_.cache_use_real_size,
        options_->opEp_.should_recovery_hotness);
  }
}
...
Status TableCache::FindTable(uint64_t file_number, uint64_t file_size,
                             Cache::Handle **handle, bool Get, int file_level,
                             TableMetaData *tableMetaData,
                             std::vector<uint64_t> *input_0_numbers,
                             std::vector<uint64_t> *input_1_numbers) {
  Status s;
  ...
  *handle = cache_->Lookup(key, false);  // Get
  // MeasureTime(Statistics::GetStatistics().get(),Tickers::MQ_LOOKUP_TIME,Env::Default()->NowMicros()
  // - start_micros_l);

  if (*handle == NULL) {
    uint32_t hash =
        Hash((const char *)&file_number, sizeof(file_number), 0) % 64;
    mutex_[hash].lock();
    ...
    *handle = cache_->Lookup(key, false);
    ...
    if (!s.ok()) {
      std::string old_fname = SSTTableFileName(dbname_, file_number);
      if (env_->NewRandomAccessFile(old_fname, &file, directIO_of_RandomAccess)
              .ok()) {
        s = Status::OK();
      }
    }
    ...
    if (input_0_numbers != NULL) {
      ...
      int table_nums =
          (input_1_numbers == NULL)
              ? (*input_0_numbers).size()
              : (*input_0_numbers).size() + (*input_1_numbers).size();
      for (int i = 0; i < table_nums; i++) {
        ...
        *id_ = 0;
        ...
        if (p_0_handle != NULL) {
          Table *p_0_table =
              reinterpret_cast<TableAndFile *>(cache_->Value(p_0_handle))
                  ->table;
      ...
      Table::getRegionKeyRangesByStr(options_, tableMetaData->index_data,
                                     region_keys_cur);
      ...
      for (int i = 0; i < freq_count; i++) {
        ...
        for (int j = 0; j < region_keys_all.size(); j++) {
          ...
          if (cur_region >= freq_count_all[j]) continue;
          Slice t_left_key = region_keys_all[j][cur_region];
          if (options_->comparator->Compare(region_keys_cur[i + 1],
                                            t_left_key) <= 0)
            continue;
          Slice t_right_key = region_keys_all[j][cur_region + 1];
          while (options_->comparator->Compare(t_right_key,
                                               region_keys_cur[i]) <= 0) {
            cur_region += 1;
            region_iters[j] += 1;
            if (cur_region >= freq_count_all[j]) break;
            t_right_key = region_keys_all[j][cur_region + 1];
          }
          if (cur_region >= freq_count_all[j]) continue;

          freqs[i] += freqs_all[j][cur_region];
          max_freq = max_freq > freqs_all[j][cur_region]
                         ? max_freq
                         : freqs_all[j][cur_region];
          overlapped++;
        }
#ifdef DEBUG_HOTNESS_INHERIT
        ...
        double freq_deviation =
            freqs[i] == 0 ? 0 : (max_freq - freqs[i]) * 1.0 / freqs[i];
        fprintf(hot_dev, "%lf,", freq_deviation);
        if (hot_counter++ % 100 == 0) fflush(hot_dev);
      ...
      for (auto it = region_keys_all.begin(); it != region_keys_all.end();
           it++) {
        clearVectorSlice(*it);
      }
    ...
    size_t *charge = NULL;
    if (s.ok()) {
      s = Table::Open(*options_, file, file_size, &table, charge, file_level,
                      tableMetaData);
      // s = Table::Open(*options_, file, file_size, &table, charge, file_level,
      // NULL);
    }
    MeasureTime(Statistics::GetStatistics().get(), Tickers::OPEN_TABLE_TIME,
    ...
    if (!s.ok()) {
      ...
      // regionNum);fflush(stdout);

      int regionNum = table->getRegionNum();

      if (charge == NULL) charge = new size_t[regionNum]();
        ...
        if (i == 0)
          *handle = cache_->Insert(key2, tf, 0, &DeleteEntry);
          ...
          /*          if (file_level == 0)
            ...
            cache_->SetFreCount(key2, total_freq);// cache_->SetFreCount(key2,
          freqs[i - 1]);
*/
        }
...
Iterator *TableCache::NewIterator(const ReadOptions &options,
  ...
  Cache::Handle *handle = NULL;
  Status s =
      FindTable(file_number, file_size, &handle, false, options.file_level,
                tableMetaData_, input_0_numbers, input_1_numbers);
  if (!s.ok()) {
    return NewErrorIterator(s);
  }
...
void TableCache::TurnOnAdjustment() { cache_->TurnOnAdjustment(); }

void TableCache::TurnOffAdjustment() { cache_->TurnOffAdjustment(); }
...
Status TableCache::Get(const ReadOptions &options, uint64_t file_number,
  ...
  // options_->opEp_.add_filter = file_access_time >
  // cache_->GetLRUFreCount()?true:false; options_->opEp_.add_filter = true;
  ...
  Status s =
      FindTable(file_number, file_size, &handle, true, options.file_level);
  MeasureTime(Statistics::GetStatistics().get(), Tickers::FINDTABLE,
              Env::Default()->NowMicros() - start_micros);
  if (s.ok()) {
    ...
    *id_ = (uint32_t)(-1);
    ...
    MeasureTime(Statistics::GetStatistics().get(), Tickers::INTERNALGET,
                Env::Default()->NowMicros() - start_micros);
    start_micros = env_->NowMicros();
    if (*id_ != (uint32_t)-1) {
      Slice key(buf, sizeof(buf));
    ...
    MeasureTime(Statistics::GetStatistics().get(), Tickers::MQ_LOOKUP_TIME,
                Env::Default()->NowMicros() - start_micros);

    start_micros = env_->NowMicros();
    cache_->Release(handle);
    MeasureTime(Statistics::GetStatistics().get(), Tickers::RELEASE,
                Env::Default()->NowMicros() - start_micros);
  }
  return s;
}
...
size_t TableCache::GetTableCurrFiltersSize(uint64_t file_number,
                                           uint64_t file_size) {
  Cache::Handle *handle = NULL;
  Status s = FindTable(file_number, file_size, &handle);
  if (s.ok()) {
...
std::string TableCache::LRU_Status() { return cache_->LRU_Status(); }
