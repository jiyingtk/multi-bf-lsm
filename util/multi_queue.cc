...
class HandleTable  // a list store LRUQueueHandle's address , don't care queue
                   // id
{
 public:
...
class MultiQueue : public Cache {
  ...
  std::vector<size_t> bits_per_key_per_filter_,
      bits_per_key_per_filter_sum;  // begin from 0 bits
  bool need_adjust;
 ...
 public:
  ...
  MultiQueue(const std::string &dbname, size_t capacity, int lrus_num = 1,
             uint64_t life_time = 20000, double cr = 0.0001,
             bool cache_use_real_size = true,
             bool should_recovery_hotness = false);
  ~MultiQueue();
  ...
  virtual Handle *Insert(const Slice &key, void *value, size_t charge,
                         void (*deleter)(const Slice &key, void *value,
                                         const bool realDelete)) override;
  ...
  virtual void inline addCurrentTime() override { ++current_time_; }
...
MultiQueue::MultiQueue(const std::string &dbname, size_t capacity, int lrus_num,
                       uint64_t life_time, double change_ratio,
                       bool cache_use_real_size, bool should_recovery_hotness)
    : dbname_(dbname),
      capacity_(capacity),
      lrus_num_(lrus_num),
      ...
      insert_count(0),
      need_adjust(true),
      cache_use_real_size_(cache_use_real_size),
      should_recovery_hotness_(should_recovery_hotness),
      check_hotness_failed_nums_(0) {
  // TODO: declare outside  class  in_use and lrus parent must be
  // Initialized,avoid lock crush
  ...
  for (int i = 1; i < lrus_num; ++i) {
    ...
    bits_per_key_per_filter_.push_back(
  ...
  if (should_recovery_hotness_) restore_all();
  ...
  for (i = 0; i < bg_thread_nums; i++) {
    if (i == 0 &&
        pthread_create(&pids_[i], NULL, MultiQueue::BackThreadAll, this) != 0) {
      perror("create thread ");
    }
    if (i == 1 && pthread_create(&pids_[i], NULL, MultiQueue::BackThreadDiff,
                                 this) != 0) {
      perror("create thread ");
    }
...
inline bool restore_freq_info(FILE *fp,
                              std::unordered_map<uint64_t, HandlePair> &map) {
  HandlePair hp;
  ...
  // varied decoding
  char buf[32];
  if (fread(buf, 1, 1, fp) != 1) return true;
  int read_count = buf[0];
  if (fread(buf, 1, read_count, fp) != read_count) return true;
...
void *MultiQueue::BackThreadAll(void *arg) {
  ...
  while (!mq->shutting_down_) {
    ...
    mq->backup_all();
    MeasureTime(Statistics::GetStatistics().get(), Tickers::MQ_BACKUP_TIME,
...
void MultiQueue::backup_all() {
  ...
  for (int i = 0; i < lrus_num_; i++) {
    ...
    for (LRUQueueHandle *e = lrus_[i].next; e != &lrus_[i];) {
      if (e->value_id != 0) {
        leveldb::TableAndFile *tf =
            reinterpret_cast<leveldb::TableAndFile *>(e->value);
        HandlePair hp = {(uint32_t)tf->file_number, e->value_id, e->fre_count,
                         e->queue_id};
        q.push(hp);
...
inline void MultiQueue::backup_node_hotness(LRUQueueHandle *e,
                                            bool shouldBackup) {
...
std::string MultiQueue::LRU_Status() {
  ...
  snprintf(
      buf, sizeof(buf),
      "MQ state: capacity: %zu usage: %zu, expection: %lf, insert_count %d\n",
      capacity_, usage_.load(std::memory_order_relaxed), expection_,
      insert_count);
  value.append(buf);
  mutex_.unlock();
  return value;
...
void MultiQueue::RecomputeExp(LRUQueueHandle *e) {
  if (multi_queue_init || (e->queue_id + 1) == lrus_num_) {
    ...
    const double new_expection =
        expection_ - (e->fre_count - 1) * FalsePositive(e) +
        e->fre_count * fps[e->queue_id + 1];  // TODO: OPTIMIZE
    int need_bits =
        usage_ - capacity_;  // + bits_per_key_per_filter_[e->queue_id + 1];
    if (e->queue_id == 0) {
      need_bits += lrus_[1].next->charge;
    } else {
      need_bits += e->charge / bits_per_key_per_filter_sum[e->queue_id] *
                   bits_per_key_per_filter_[e->queue_id + 1];
    }
    ...
    if (need_bits > 0) {
      ...
      for (int i = 1; i < lrus_num_; i++) {
        ...
        while (old != &lrus_[i] && remove_bits < need_bits) {
          if (old->expire_time < current_time_)  // expired
          {
            ...
            remove_bits += old->charge /
                           bits_per_key_per_filter_sum[old->queue_id] *
                           bits_per_key_per_filter_[i];
            change_expection += (old->fre_count * fps[i - 1] -
                                 old->fre_count * FalsePositive(old));
          } else {
            break;
          }
          old = old->next;
      ...
      if (min_i != -1) counters[10]++;
      if (min_i != -1 &&
          now_expection - min_expection > now_expection * change_ratio_) {
        counters[3]++;
...
void MultiQueue::Unref(LRUQueueHandle *e) {
  ...
  deallocate:
    ...
    free(e);
             ...
             e->refs == 1)  // note:No longer in use; move to lru_ list.
  {
    leveldb::TableAndFile *tf =
        reinterpret_cast<leveldb::TableAndFile *>(e->value);
    if (e->value_id > 0 && tf->table->getCurrFilterNum(e->value_id - 1) <
                               e->queue_id)  // only add;
    {
      ++e->refs;
      size_t delta_charge = tf->table->AdjustFilters(
          e->queue_id, e->value_id - 1);  // not in lru list, so need to worry
                                          // will be catched by ShrinkUsage
      --e->refs;
      if (e->refs == 0) {
        goto deallocate;
      }
      ...
      MayBeShrinkUsage();
    } else if (e->value_id > 0 &&
               tf->table->getCurrFilterNum(e->value_id - 1) > e->queue_id) {
...
void MultiQueue::LRU_Append(LRUQueueHandle *list, LRUQueueHandle *e) {
  ...
  e->prev->next = e;
  e->next->prev = e;
  // if append to in_use , no need to remember queue_id,thus in_use_mutex used
  // independently
    ...
    while (usage_ > capacity_ && k >= 1) {
      while (lrus_[k].next != &lrus_[k] && removed_usage < remove_charge[0]) {
        ...
        if (old->refs == 1 && old->expire_time < current_time_) {
          ...
          size_t delta_charge = tf->table->RemoveFilters(1, old->value_id - 1);
          MeasureTime(Statistics::GetStatistics().get(),
                      Tickers::REMOVE_EXPIRED_FILTER_TIME_0 + k,
                      Env::Default()->NowMicros() - start_micros);
          ...
          old->fre_count =
              Num_Queue(k - 1, old->fre_count);  // also decrease fre count
    ...
    if (max_i != -1) {
      ...
      while (removed_usage < remove_charge[0]) {
        ...
        leveldb::TableAndFile *tf =
            reinterpret_cast<leveldb::TableAndFile *>(old->value);
        size_t delta_charge =
            tf->table->RemoveFilters(1, old->value_id - 1);  //  remove 1 filter
        old->charge -= delta_charge;
        usage_ -= delta_charge;
        removed_usage += delta_charge;
        expection_ -= old->fre_count * fps[k];
        ...
        sum_freqs_[k - 1] += old->fre_count;
        MeasureTime(Statistics::GetStatistics().get(),
                    Tickers::REMOVE_HEAD_FILTER_TIME_0 + k,
                    Env::Default()->NowMicros() - start_micros);
        backup_node_hotness(old);
      }
    }
...
int MultiQueue::AllocFilterNums(int freq) {
  ...
  while (i < lrus_num_) {
    if (lrus_[i].next != &lrus_[i]) {
      if (freq < sum_freqs_[i] / lru_lens_[i]) return i;
      last_ = i;
    }
    ++i;
  }
...
Cache::Handle *MultiQueue::Insert(const Slice &key, void *value, size_t charge,
                                  void (*deleter)(const Slice &key, void *value,
...
uint64_t MultiQueue::NewId() { return ++(last_id_); }
...
Cache *NewMultiQueue(const std::string &dbname, size_t capacity, int lrus_num,
                     uint64_t life_time, double change_ratio,
                     bool cache_use_real_size, bool should_recovery_hotness) {
  return new multiqueue_ns::MultiQueue(dbname, capacity, lrus_num, life_time,
                                       change_ratio, cache_use_real_size,
                                       should_recovery_hotness);
}
