...
Status ReadBlocks(RandomAccessFile *file, const ReadOptions &options,
                  const BlockHandle *handles_ptr, BlockContents *results,
                  int n) {
  int i;
  ...
  Slice contents[32];
  Status s =
      file->Reads(handles_ptr[0].offset(), sum_lens + n * kBlockTrailerSize,
                  contents, bufs, lens, n);
  if (!s.ok()) {
    std::cout << "ReadBlocks read failed, n " << n << " offset "
              << handles_ptr[0].offset() << " len0 " << lens[0] << " status "
              << s.ToString() << std::endl;
  ...
  for (i = 0; i < n; i++) {
    ...
    switch (data[handles_ptr[i].size()]) {
      ...
      case kSnappyCompression: {
        size_t ulength = 0;
        if (!port::Snappy_GetUncompressedLength(data, handles_ptr[i].size(),
                                                &ulength)) {
          delete[] bufs[i];
...
Status ReadBlock(RandomAccessFile *file, const ReadOptions &options,
  ...
  MeasureTime(Statistics::GetStatistics().get(), Tickers::READ_BLOCK_NEW_TIME,
  ...
  MeasureTime(Statistics::GetStatistics().get(),
              Tickers::READ_BLOCK_FILE_READ_TIME,
              Env::Default()->NowMicros() - start_micros);
  if (!s.ok()) {
    delete[] buf;
    return s;
