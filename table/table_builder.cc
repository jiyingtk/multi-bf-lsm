...
Status TableBuilder::Finish() {
  ...
  BlockHandle temp_filter_block_handle, metaindex_block_handle,
      index_block_handle;
  ...
  if (ok() && r->filter_block != NULL) {
    ...
    int i = 0;
    for (auto results_iter = results.begin(); results_iter != results.end();
         results_iter++) {
      ...
      WriteRawBlock(rawSlice, kNoCompression, &temp_filter_block_handle);
      filter_block_handles.push_back(temp_filter_block_handle);
    }
    MeasureTime(Statistics::GetStatistics().get(), Tickers::WRITE_FILTER_TIME,
  ...
  if (ok()) {
    ...
    if (r->filter_block != NULL) {
      ...
      while (FilterPolicy::bits_per_key_per_filter_[i] != 0) {
        while (FilterPolicy::bits_per_key_per_filter_[i + 1] != 0 &&
               FilterPolicy::bits_per_key_per_filter_[i] ==
                   FilterPolicy::bits_per_key_per_filter_[i + 1])
          i++;

        std::string key =
            "filter.0offsets" + std::to_string(i) + "filter.offset";
      ...
      char id[] = {'1', 0};
      for (auto handle_iter = filter_block_handles.begin();
           handle_iter != filter_block_handles.end(); handle_iter++) {
        std::string key =
            "filter." + std::string(id);  // TODO::key may should add filter id
